...
class DriverNeededDocumentTransformer extends Transformer
{
    ...
    */
    ...
    public function transform(DriverNeededDocument $driverneededdocument)
    {
        ...
        $driverId = request()->get('driver_id') ?? (auth()->user()->driver->id ?? null);

        $driver_document = null;
        if ($driverId) {
            $driver_document = DriverDocument::where('document_id', $driverneededdocument->id)->where('driver_id', $driverId)->first();
        }
    ...
    public function includeDriverDocument(DriverNeededDocument $driverneededdocument)
    {
        ...
        $roles = null;
        if ($driverId) {
            $roles = $driverneededdocument->driverDocument()->where('driver_id', $driverId)->first();
        }
